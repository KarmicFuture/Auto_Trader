...
def test_resolve_source_name_from_label_and_host():
    ...
    assert (
        resolve_source_name(
            {"sourceName": "Carvana", "url": "https://www.carvana.com/vehicle/123"}
        )
        == "carvana"
    )
...
def test_to_listing_carvana():
    listing = AutoTempestClient._to_listing(
        {
            "id": "cv-9",
            "title": "2016 Porsche Cayman Coupe 2D",
            "url": "https://www.carvana.com/vehicle/123?utm=x",
            "sourceName": "Carvana",
            "price": "$42,990",
            "year": "2016",
            "mileage": "41,200",
    ...
    assert listing is not None
    assert listing.source == "carvana"
    assert listing.url == "https://www.carvana.com/vehicle/123"
    assert listing.price == 42990
